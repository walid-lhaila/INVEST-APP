...
import {StatusBar, View, TouchableOpacity, ScrollView, Image, Text, StyleSheet, KeyboardAvoidingView, Platform, Pressable, ActivityIndicator} from "react-native";
...
import {clearCities} from "@/app/redux/slices/CitiesSlice";
...
function UpdatePostForm({onClose, postId}) {
    const dispatch = useDispatch();
    const { imageUri, pickImage, setImageUri } = useImagePicker();
    const { form, handleChange, handleSubmit } = useUpdatePost(onClose, imageUri, postId);
    ...
    useEffect(() => {
        ...
        if (post) {
            handleChange('title', post.title);
            handleChange('description', post.description);
            handleChange('investmentGoal', post.investmentGoal.toString());
            handleChange('currentInvestment', post.currentInvestment.toString());
            setQuery(post.location);
            handleChange('tags', post.tags.join(', '));
            handleChange('category', post.category.join(', '));
            handleChange('status', post.status);
            if (post.imageUrl) {
                const fixedImageUrl = post.imageUrl.replace("0.0.0.0", process.env.EXPO_PUBLIC_MINIO_URL);
                setImageUri(fixedImageUrl);
            }
        }
    }, [postId, posts]);
                ...
                >
                    ...
                    <ScrollView contentContainerStyle={styles.content}>
                        ...
                        <View style={styles.autocompleteContainer}>
                            <AutocompleteInput data={cities} value={query} onChangeText={(text) => {setQuery(text);handleChange('location', text);}} placeholder="Location"
